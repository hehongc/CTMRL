...
from rlkit.torch.New_networks import VectorQuantize, NewSelfAttnEncoder
...
def experiment(variant, seed=None):
    ...
    VectorQuantizeCodebook = VectorQuantize(
        input_dim=context_encoder_output_dim,
        codebook_size=variant['codebook_size'],
        code_dim=context_encoder_output_dim,
        decay=0.9,
...
@click.option("--codebook_size", default=2)
def main(config, data_dir, gpu, is_sparse_reward, use_brac, use_information_bottleneck, is_zloss, is_onlineadapt_thres,
         is_onlineadapt_max, num_exp_traj_eval, allow_backward_z, is_true_sparse_rewards, r_thres, wandb_project_name,
         wandb_run_name, csv_name, codebook_size, num_ensemble):
    ...
    variant['codebook_size'] = codebook_size
