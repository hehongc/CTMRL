...
class OfflineMetaRLAlgorithm(metaclass=abc.ABCMeta):
    ...
    def _try_to_eval(self, epoch):
        if self._can_evaluate():
            ...
            avg_train_online_all_return_one_shot, avg_train_offline_all_return_one_shot, avg_train_np_online_all_return_one_shot = self.evaluate(
                epoch)
    ...
    def train(self):
        ...
        for it_ in gt.timed_for(range(self.num_iterations), save_itrs=True):
            ...
            for train_step in range(self.num_train_steps_per_itr):
                Codebook_before_train = self.agent.VectorQuantizeCodebook.codebook
                ...
                if (it_ / self.num_iterations) < 0.3:
                    use_VQ = False
                    z_means, z_vars, wandb_stat = self._do_training(indices, use_VQ=False)
                else:
                    use_VQ = True
                    z_means, z_vars, wandb_stat = self._do_training(indices, use_VQ=True)
            ...
            # warming
            if (it_ / self.num_iterations) < 0.2:
                # pass
                all_context = None
            # collect context for initiate
            elif (it_ / self.num_iterations) > 0.2 and (it_ / self.num_iterations) < 0.3:
                self.agent.collect_context_embeddings(all_context)
            # initiate Codebook
            elif ((it_ - 1) / self.num_iterations) < 0.3 and (it_ / self.num_iterations) >= 0.3:
                self.agent.clear_collected_context_embeddings()
                Kmeans_update_count = 0
            # update codebook
            elif (it_ / self.num_iterations) > 0.3:
                if Kmeans_update_count == int(self.num_iterations // 20) and (it_ / self.num_iterations) > 0.3:
                    Kmeans_update_count = 0
                    self.agent.VectorQuantizeCodebook.Kmeans_update_VQ(self.agent.collected_context_embeddings.cuda())
                    self.agent.clear_collected_context_embeddings()
                else:
                    Kmeans_update_count = Kmeans_update_count + 1
                    self.agent.collect_context_embeddings(all_context)
            ...
            if self.allow_eval:
                ...
                avg_train_online_all_return, avg_train_offline_all_return, avg_train_np_online_all_return, \
                avg_test_online_all_return_one_shot, avg_test_offline_all_return_one_shot, avg_test_np_online_all_return_one_shot, \
                avg_train_online_all_return_one_shot, avg_train_offline_all_return_one_shot, avg_train_np_online_all_return_one_shot = self._try_to_eval(
                    it_)
                ...
                # wandb_stat["avg_test_offline_all_return_one_shot"] = np.mean(
                #     avg_test_offline_all_return_one_shot).item()
                # wandb_stat["avg_test_np_online_all_return_one_shot"] = np.mean(
                #     avg_test_np_online_all_return_one_shot).item()
                #
                # wandb_stat["avg_train_online_all_return_one_shot"] = np.mean(
                #     avg_train_online_all_return_one_shot).item()
                # wandb_stat["avg_train_offline_all_return_one_shot"] = np.mean(
                #     avg_train_offline_all_return_one_shot).item()
                # wandb_stat["avg_train_np_online_all_return_one_shot"] = np.mean(
                #     avg_train_np_online_all_return_one_shot).item()
    ...
    def evaluate(self, epoch):
        ...
        test_online_all_returns_one_shot, test_offline_all_returns_one_shot, test_np_online_all_returns_one_shot = self._do_eval(
        ...
        train_online_all_returns_one_shot, train_offline_all_returns_one_shot, train_np_online_all_returns_one_shot = self._do_eval(
            indices, epoch, buffer=self.replay_buffer)
        ...
        ######################################################################
        self.loss["train_returns"] = np.mean(avg_test_online_final_return).item()
        self.loss["avg_train_return"] = np.mean(avg_test_offline_final_return).item()
        self.loss["avg_test_return"] = np.mean(avg_test_np_online_final_return).item()

        self.loss["avg_test_online_all_return"] = np.mean(avg_test_online_all_return).item()
        self.loss["avg_test_offline_all_return"] = np.mean(avg_test_offline_all_return).item()
        self.loss["avg_test_np_online_all_return"] = np.mean(avg_test_np_online_all_return).item()
        self.loss["avg_train_online_final_return"] = np.mean(avg_train_online_final_return).item()
        self.loss["avg_train_offline_final_return"] = np.mean(avg_train_offline_final_return).item()
        self.loss["avg_train_np_online_final_return"] = np.mean(avg_train_np_online_final_return).item()
        self.loss["avg_train_online_all_return"] = np.mean(avg_train_online_all_return).item()
        self.loss["avg_train_offline_all_return"] = np.mean(avg_train_offline_all_return).item()
        self.loss["avg_train_np_online_all_return"] = np.mean(avg_train_np_online_all_return).item()

        self.loss["avg_test_online_all_return_one_shot"] = np.mean(avg_test_online_all_return_one_shot).item()
        self.loss["avg_test_offline_all_return_one_shot"] = np.mean(avg_test_offline_all_return_one_shot).item()
        self.loss["avg_test_np_online_all_return_one_shot"] = np.mean(avg_test_np_online_all_return_one_shot).item()

        self.loss["avg_train_online_all_return_one_shot"] = np.mean(avg_train_online_all_return_one_shot).item()
        self.loss["avg_train_offline_all_return_one_shot"] = np.mean(avg_train_offline_all_return_one_shot).item()
        self.loss["avg_train_np_online_all_return_one_shot"] = np.mean(avg_train_np_online_all_return_one_shot).item()
        ...
        return avg_test_online_final_return, avg_test_offline_final_return, avg_test_np_online_final_return, \
               np.mean(avg_test_online_all_return), np.mean(avg_test_offline_all_return), np.mean(
            avg_test_np_online_all_return), \
               avg_train_online_final_return, avg_train_offline_final_return, avg_train_np_online_final_return, \
               np.mean(avg_train_online_all_return), np.mean(avg_train_offline_all_return), np.mean(
            avg_train_np_online_all_return), \
               np.mean(avg_test_online_all_return_one_shot), np.mean(avg_test_offline_all_return_one_shot), np.mean(
            avg_test_np_online_all_return_one_shot), \
               np.mean(avg_train_online_all_return_one_shot), np.mean(avg_train_offline_all_return_one_shot), np.mean(
            avg_train_np_online_all_return_one_shot)
