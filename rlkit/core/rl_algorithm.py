...
class OfflineMetaRLAlgorithm(metaclass=abc.ABCMeta):
		...
		with open(csv_file_name, "w", encoding='utf-8', newline='') as f:
			...
			for it_ in gt.timed_for(range(self.num_iterations), save_itrs=True):
				...
				for train_step in range(self.num_train_steps_per_itr):
					...
					if (it_ / self.num_iterations) < 0.1:
						use_VQ = False
						z_means, z_vars, wandb_stat = self._do_training(indices, use_VQ=False, zloss=self.is_zloss)
					else:
						use_VQ = True
						z_means, z_vars, wandb_stat = self._do_training(indices, use_VQ=True, zloss=self.is_zloss)
				...
				elif ((it_ - 1) / self.num_iterations) < 0.1 and (it_ / self.num_iterations) >= 0.1:
					self.agent.clear_collected_context_embeddings()
					Kmeans_update_count = 0
				else:
					if Kmeans_update_count == int(self.num_iterations / 10) and (it_ / self.num_iterations) > 0.1:
						Kmeans_update_count = 0
						self.agent.VectorQuantizeCodebook.Kmeans_update_VQ(self.agent.collected_context_embeddings)
						self.agent.clear_collected_context_embeddings()
					else:
	...
	@abc.abstractmethod
	def take_all_training_tasks_contexts(self):
		pass

	# @abc.abstractmethod
	# def take_all_training_tasks_code_scores(self):
	# 	pass
